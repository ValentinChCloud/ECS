return {
  ...
  new = function (name, requires, priority)
    ...
    local system = {
      requires = requires,
      __name = assert.type(name, "string"),
      __type = "system",
      __priority = assert.int(priority or 10),
      list = {}
    }
    ...
    function system:draw() 
      if self.core_draw == nil then
        return
      end
      for i = 1, #self.list do
        self:core_draw(self.list[i])
      end
    end
    ...
    function system:core_update(dt,entity) end
